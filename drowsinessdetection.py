...
alarm_sound_played = False  # Initialize the alarm flag
...
while(True):
    ...
    for (x,y,w,h) in right_eye:
        ...
        r_pred_raw = model.predict(r_eye)
        rpred = np.argmax(r_pred_raw, axis=-1)
        if(rpred[0]==1):
            lbl='Open'
        if(rpred[0]==0):
            lbl='Closed'
    ...
    for (x,y,w,h) in left_eye:
        ...
        l_pred_raw = model.predict(l_eye)
        lpred = np.argmax(l_pred_raw, axis=-1)
        if(lpred[0]==1):
            lbl='Open'
        if(lpred[0]==0):
            lbl='Closed'
    ...
    elif(score<5):  # Reset the alarm flag if score drops below 5
